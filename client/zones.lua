...
local function ZoneAlert()
	...
	local time = QRCore.Functions.GetCurrentTime().time
	local temp = QRCore.Functions.GetTemperature()
	if zone then
      QRCore.Functions.NativeNotify(6, time..' '..temp, 4000, zone, 'generic_textures', 'tick', "COLOR_YELLOW")
   end
end
